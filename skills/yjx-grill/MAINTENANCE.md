...
1. **User Commitments & Core Model (用户决策与核心模型)**: Verifies what the human explicitly committed to (P0 / highest weight). Expressed as a clean **Native Markdown Nested Tree** across all business scenarios (Trigger / Result) without ASCII/Unicode box-drawing artifacts. No agent-inferred rules belong here.
