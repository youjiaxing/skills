...
*Completion Criterion*: Required behavior implemented; changes causally bounded; every new indirection has a production responsibility that remains useful without its tests; the test decision is recorded; relevant compatibility and resource-cost implications assessed.
...
1. **Independent Review**: Use `/code-review` for parallel independent Standards and Spec reviews of the complete task changes. Both axes are required for every implementation; primary-agent risk classification cannot reduce either axis. Add this skill directly to the Standards Reviewer prompt as an explicit standards source, alongside the project standards. Review correctness and regression risks against those standards, the originating requirements, and any independently reviewed alignment contract.
