...
**字符网格呈现目标：** 列表行为 `mark + id + [type] + 状态显示名 + 标题截断`，并让 `◀选中`、`◀当前槽`、`←看板默认` 可分；默认中带只展开可执行列表 + 焦点邻域（全板其余折叠，完整花名册在 `g`）；终端够高时邻域用多行真连线，过矮可压缩但须明示“已降级”；三带短标题、主 CTA、选中/当前槽、warn、hot/dim 等语义角色须可分（`ic` dual-TTY 自动开色，无需手设 `FORCE_COLOR`；尊重 `NO_COLOR`），不锁具体 RGB；`g` 全局总览节点可辨、长依赖链竖排折行、能返回列表+邻域。

**观感完成门禁：** 字符串/帧契约与编排回归全绿后，仍须在真实 dual-TTY 全屏终端抽检高/矮布局、角色色、密度与 `g` 往返，并由操作者确认“同方向、可日常”；测试绿不能单独代表全屏观感完成。
