...
| 调度伙伴 | `yjx-local-ralph` | `yjx-gh-ralph`（手动单票，无 auto-run） |

依赖分层：

- `yjx-local-tracker-setup` / `yjx-local-kanban` / `yjx-local-ralph` / `yjx-gh-kanban` / `yjx-gh-ralph`：以 **Node 标准库**为主（无 Ink）。`yjx-gh-kanban` / `yjx-gh-ralph` 另需本机 `gh`。
